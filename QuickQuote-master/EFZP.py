...
# human readers of the email text. For example, email_text may start with
# "A message from your customer:"


def strip_automated_notation(email_text):
	# Use a paramater name email_text to indicate text in the actual email
	# message
	notations = [
...
def get_signature(email_text):
	...
	# try not to have the signature be the very start of the message if we can
	# avoid it
	salutation = get_salutation(email_text)
	if salutation:
	...
	# try a probabilistic approach by looking for phone numbers, names etc. to
	# derive sig
	if not signature:
...
# todo: complete this out (I bit off a bit more than I could chew with
# this function. Will probably take a bunch of basian stuff
